...
void QCPBarsGroup::remove(QCPBars* bars)
{
    ...
    if (mBars.contains(bars))
        ...
        qDebug() << Q_FUNC_INFO
                 << "bars plottable is not in this bars group:" << reinterpret_cast<quintptr>(bars);
}
...
double QCPBarsGroup::keyPixelOffset(const QCPBars* bars, double keyCoord)
{
    ...
    // determine key pixel offset of this base bars considering all other base bars in this
    // barsgroup:
    double result = 0;
    ...
    if (index >= 0)
    {
        if (baseBars.size() % 2 == 1
            && index == (baseBars.size() - 1) / 2) // is center bar (int division on purpose)
        {
            return result;
        }
        else
...
/*! \overload
  ...
  \see addData
*/
void QCPBars::setData(const QVector<double>& keys, const QVector<double>& values,
                      bool alreadySorted)
{
...
QCPRange QCPBars::getKeyRange(bool& foundRange, QCP::SignDomain inSignDomain) const
{
    ...
    spans than before, which in turn would require a different key range to perfectly fit, and so
    on. The only solution would be to iteratively approach the perfect fitting axis range, but the
...
QPointF QCPBars::dataPixelPosition(int index) const
{
    if (index >= 0 && index < mDataContainer->size())
    {
        ...
        const double valuePixel
            = valueAxis->coordToPixel(getStackedBaseValue(it->key, it->value >= 0) + it->value);
                                   ...
                                   QCPBarsDataContainer::const_iterator& end) const
{
    ...
    // walk left from begin to find lower bar that actually is completely outside visible pixel
    // range:
    ...
    // walk right from ubound to find upper bar that actually is completely outside visible pixel
    // range:
    it = end;
...
QRectF QCPBars::getBarRect(double key, double value) const
{
    ...
    double bottomOffset
        = (mBarBelow && mPen != Qt::NoPen ? 1 : 0) * (mPen.isCosmetic() ? 1 : mPen.widthF());
    ...
    switch (mWidthType)
    {
        ...
        case wtPlotCoords:
        {
            if (mKeyAxis)
            {
                ...
                // no need to qSwap(lower, higher) when range reversed, because higher/lower are
                // gained by coordinate transform which includes range direction
            }
            else
...
double QCPBars::getStackedBaseValue(double key, bool positive) const
{
    if (mBarBelow)
    {
        double max = 0; // don't initialize with mBaseValue here because only base value of
                        // bottom-most bar has meaning in a bar stack
        // find bars of mBarBelow that are approximately at key and find largest one:
        double epsilon = qAbs(key)
            * (sizeof(key) == 4
                   ? 1e-6
                   : 1e-14); // should be safe even when changed to use float at some point
        if (key == 0)
