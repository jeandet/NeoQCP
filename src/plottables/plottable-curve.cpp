...
/*! \class QCPCurveData
  ...
  The stored data is:
  \li \a t: the free ordering parameter of this curve point, like in the mathematical vector
  <em>(x(t), y(t))</em>. (This is the \a sortKey)
...
/*! \class QCPCurve
  ...
  (<tt>qQNaN()</tt> or <tt>std::numeric_limits<double>::quiet_NaN()</tt>) in between the two data
  points that shall be separated.
...
QCPCurve::QCPCurve(QCPAxis* keyAxis, QCPAxis* valueAxis)
        : QCPAbstractPlottable1D<QCPCurveData>(keyAxis, valueAxis), mScatterSkip {}, mLineStyle {}
{
                       ...
                       const QVector<double>& values, bool alreadySorted)
{
    if (t.size() != keys.size() || t.size() != values.size())
        qDebug() << Q_FUNC_INFO << "ts, keys and values have different sizes:" << t.size()
...
void QCPCurve::addData(const QVector<double>& keys, const QVector<double>& values)
{
    if (keys.size() != values.size())
        qDebug() << Q_FUNC_INFO << "keys and values have different sizes:" << keys.size()
                 << values.size();
...
void QCPCurve::draw(QCPPainter* painter)
{
    ...
    for (int i = 0; i < allSegments.size(); ++i)
    {
        ...
        QPen finalCurvePen = mPen; // determine the final pen already here, because the line
        ...
        for (QCPCurveDataContainer::const_iterator it = mDataContainer->constBegin();
...
void QCPCurve::drawLegendIcon(QCPPainter* painter, const QRectF& rect) const
{
    ...
    {
        applyFillAntialiasingHint(painter);
        painter->fillRect(QRectF(rect.left(), rect.top() + rect.height() / 2.0, rect.width(),
                                 rect.height() / 3.0),
                          mBrush);
    }
...
*/
void QCPCurve::drawScatterPlot(QCPPainter* painter, const QVector<QPointF>& points,
...
*/
void QCPCurve::getCurveLines(QVector<QPointF>* lines, const QCPDataRange& dataRange,
                             double penWidth) const
{
    ...
    const double valueMin
        = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().lower)
                                  - strokeMargin * valueAxis->pixelOrientation());
    const double valueMax
        = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().upper)
                                  + strokeMargin * valueAxis->pixelOrientation());
    ...
    QVector<QPointF> trailingPoints; // points that must be applied after all other points (are
                                     // generated only when handling first point to get virtual
                                     // segment between last and first point right)
    while (it != itEnd)
    {
        const int currentRegion = getRegion(it->key, it->value, keyMin, valueMax, keyMax, valueMin);
        if (currentRegion != prevRegion) // changed region, possibly need to add some optimized edge
                                         // points or original points if entering R
        {
            if (currentRegion != 5) // segment doesn't end in R, so it's a candidate for removal
            {
                ...
                {
                    lines->append(getOptimizedPoint(currentRegion, it->key, it->value, prevIt->key,
                                                    prevIt->value, keyMin, valueMax, keyMax,
                                                    valueMin));
                    // in the situations 5->1/7/9/3 the segment may leave R and directly cross
                    // through two outer regions. In these cases we need to add an additional corner
                    // point
                    *lines << getOptimizedCornerPoints(prevRegion, currentRegion, prevIt->key,
                                                       prevIt->value, it->key, it->value, keyMin,
                                                       valueMax, keyMax, valueMin);
                }
                else if (mayTraverse(prevRegion, currentRegion)
                         && getTraverse(prevIt->key, prevIt->value, it->key, it->value, keyMin,
                                        valueMax, keyMax, valueMin, crossA, crossB))
                {
                    // add the two cross points optimized if segment crosses R and if segment isn't
                    // virtual zeroth segment between last and first curve point:
                    QVector<QPointF> beforeTraverseCornerPoints, afterTraverseCornerPoints;
                    getTraverseCornerPoints(prevRegion, currentRegion, keyMin, valueMax, keyMax,
                                            valueMin, beforeTraverseCornerPoints,
                ...
                else // doesn't cross R, line is just moving around in outside regions, so only need
                     // to add optimized point(s) at the boundary corner(s)
                {
                    *lines << getOptimizedCornerPoints(prevRegion, currentRegion, prevIt->key,
                                                       prevIt->value, it->key, it->value, keyMin,
                                                       valueMax, keyMax, valueMin);
            ...
            else // segment does end in R, so we add previous point optimized and this point at
                 // original position
            {
                if (it == itBegin) // it is first point in curve and prevIt is last one. So save
                                   // optimized point for adding it to the lineData in the end
                    trailingPoints
                        << getOptimizedPoint(prevRegion, prevIt->key, prevIt->value, it->key,
                                             it->value, keyMin, valueMax, keyMax, valueMin);
                else
                    lines->append(getOptimizedPoint(prevRegion, prevIt->key, prevIt->value, it->key,
                                                    it->value, keyMin, valueMax, keyMax, valueMin));
...
*/
void QCPCurve::getScatters(QVector<QPointF>* scatters, const QCPDataRange& dataRange,
                           double scatterWidth) const
{
    ...
    keyRange.lower = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyRange.lower)
                                           - scatterWidth * keyAxis->pixelOrientation());
    keyRange.upper = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyRange.upper)
                                           + scatterWidth * keyAxis->pixelOrientation());
    ...
    if (keyAxis->orientation() == Qt::Vertical)
    {
        while (it != end)
        {
            if (!qIsNaN(it->value) && keyRange.contains(it->key) && valueRange.contains(it->value))
                scatters->append(
                    QPointF(valueAxis->coordToPixel(it->value), keyAxis->coordToPixel(it->key)));
    ...
    else
    {
        while (it != end)
        {
            if (!qIsNaN(it->value) && keyRange.contains(it->key) && valueRange.contains(it->value))
                scatters->append(
                    QPointF(keyAxis->coordToPixel(it->key), valueAxis->coordToPixel(it->value)));
...
*/
int QCPCurve::getRegion(double key, double value, double keyMin, double valueMax, double keyMax,
...
/*! \internal
  ...
  equal to 5.
*/
QPointF QCPCurve::getOptimizedPoint(int otherRegion, double otherKey, double otherValue, double key,
    ...
    switch (otherRegion)
    {
        case 1: // top and left edge
        {
            ...
            if (intersectKeyPx < qMin(keyMinPx, keyMaxPx)
                || intersectKeyPx
                    > qMax(keyMinPx,
                           keyMaxPx)) // check whether top edge is not intersected, then it must be
                                      // left edge (qMin/qMax necessary since axes may be reversed)
            {
        ...
        case 3: // bottom and left edge
        {
            ...
            if (intersectKeyPx < qMin(keyMinPx, keyMaxPx)
                || intersectKeyPx > qMax(
                       keyMinPx,
                       keyMaxPx)) // check whether bottom edge is not intersected, then it must be
                                  // left edge (qMin/qMax necessary since axes may be reversed)
            {
                intersectKeyPx = keyMinPx;
                intersectValuePx = otherValuePx
        ...
        case 5:
        {
            break; // case 5 shouldn't happen for this function but we add it anyway to prevent
                   // potential discontinuity in branch table
        }
        case 6: // bottom edge
        {
            intersectValuePx = valueMinPx;
        ...
        case 7: // top and right edge
        {
            ...
            if (intersectKeyPx < qMin(keyMinPx, keyMaxPx)
                || intersectKeyPx
                    > qMax(keyMinPx,
                           keyMaxPx)) // check whether top edge is not intersected, then it must be
                                      // right edge (qMin/qMax necessary since axes may be reversed)
            {
...
/*! \internal
  ...
  traversing 5.
*/
QVector<QPointF> QCPCurve::getOptimizedCornerPoints(int prevRegion, int currentRegion,
    ...
    switch (prevRegion)
    {
        case 1:
        {
            switch (currentRegion)
            {
                ...
                case 9:
                { // in this case we need another distinction of cases: segment may pass below or
                  // above rect, requiring either bottom right or top left corner points
...
*/
bool QCPCurve::getTraverse(double prevKey, double prevValue, double key, double value,
                           ...
                           QPointF& crossA, QPointF& crossB) const
{
    ...
    else // line is skewed
    {
        ...
        if (gamma >= qMin(valueMinPx, valueMaxPx)
            && gamma
                <= qMax(valueMinPx, valueMaxPx)) // qMin/qMax necessary since axes may be reversed
            intersections.append(mKeyAxis->orientation() == Qt::Horizontal
    ...
    {
        // line probably goes through corner of rect, and we got duplicate points there. single out
        // the point pair with greatest distance in between:
        double distSqrMax = 0;
        QPointF pv1, pv2;
    ...
    {
        // one or even zero points found (shouldn't happen unless line perfectly tangent to corner),
...
/*! \internal
  ...
  segment into/out of region 5, as \ref getTraverse provides. It may happen that a single segment,
  in addition to traversing region 5, skips another region outside of region 5, which makes it
...
*/
void QCPCurve::getTraverseCornerPoints(int prevRegion, int currentRegion, double keyMin,
                               ...
                               QCPCurveDataContainer::const_iterator& closestData) const
{
    ...
    // calculate distance to line if there is one (if so, will probably be smaller than distance to
    // closest data point):
    if (mLineStyle != lsNone)
    {
        ...
        getCurveLines(&lines, QCPDataRange(0, dataCount()),
                      mParentPlot->selectionTolerance()
                          * 1.2); // optimized lines outside axis rect shouldn't respond to clicks
                                  // at the edge, so use 1.2*tolerance as pen width
        for (int i = 0; i < lines.size() - 1; ++i)
        {
            double currentDistSqr
                = QCPVector2D(pixelPoint).distanceSquaredToLine(lines.at(i), lines.at(i + 1));
            if (currentDistSqr < minDistSqr)
                minDistSqr = currentDistSqr;
        }
    }
