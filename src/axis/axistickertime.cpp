...
double QCPAxisTickerTime::getTickStep(const QCPRange& range)
{
    ...
    {
        // the filling of availableSteps seems a bit contorted but it fills in a sorted fashion and
        // thus saves a post-fill sorting run
        ...
        if (mSmallestUnit == tuMilliseconds)
            availableSteps
                << 2.5; // only allow half second steps if milliseconds are there to display it
        ...
        if (mSmallestUnit <= tuSeconds)
            availableSteps << 2.5
                    * 60; // only allow half minute steps if seconds are there to display it
        ...
        if (mSmallestUnit <= tuHours)
            availableSteps << 1 * 3600 << 2 * 3600 << 3 * 3600 << 6 * 3600 << 12 * 3600
                           << 24 * 3600;
                                        ...
                                        [[maybe_unused]] int precision)
{
    ...
    // no need to set values[tuDays] because days are always a rest value (there is no higher unit
    // so it consumes all remaining time)

    QString result = mTimeFormat;
    for (int i = mSmallestUnit; i <= mBiggestUnit; ++i)
    {
        TimeUnit iUnit = static_cast<TimeUnit>(i);
        replaceUnit(result, iUnit,
...
*/
void QCPAxisTickerTime::replaceUnit(QString& text, QCPAxisTickerTime::TimeUnit unit,
                                    int value) const
{
