...
void QCPLabelPainterPrivate::drawTickLabel(QCPPainter* painter, const QPointF& tickPos,
                                           const QString& text)
{
    double realRotation = mRotation;

    AnchorSide realSide = mAnchorSide;
    // for circular axes, the anchor side is determined depending on the quadrant of tickPos with
    // respect to mCircularReference
    if (mAnchorMode == amSkewedUpright)
    {
        realSide = skewedAnchorSide(tickPos, 0.2, 0.3);
    }
    else if (mAnchorMode == amSkewedRotated) // in this mode every label is individually rotated to
                                             // match circle tangent
    {
    ...
    realSide = rotationCorrectedSide(
        realSide, realRotation); // rotation angles may change the true anchor side of the label
    drawLabelMaybeCached(painter, mFont, mColor, getAnchorPos(tickPos), realSide, realRotation,
                         text);
}
...
tickLabelsSize.width(); result += tickLabelPadding;
    }
  }

  // calculate size of axis label (only height needed, because left/right labels are rotated by 90
degrees): if (!label.isEmpty())
  {
    QFontMetrics fontMetrics(labelFont);
    QRect bounds;
    bounds = fontMetrics.boundingRect(0, 0, 0, 0, Qt::TextDontClip | Qt::AlignHCenter |
Qt::AlignVCenter, label); result += bounds.height() + labelPadding;
  }
...
QByteArray QCPLabelPainterPrivate::generateLabelParameterHash() const
{
    ...
    // result.append(QByteArray::number(int(tickLabelSide))); TODO: check whether this is really a
    // cache-invalidating property
...
/*! \internal
  ...
  getTickLabelData).
*/
void QCPLabelPainterPrivate::drawLabelMaybeCached(QCPPainter* painter, const QFont& font,
                                                  ...
                                                  const QString& text)
{
        ...
        && !painter->modes().testFlag(QCPPainter::pmNoCaching)) // label caching enabled
    {
        ...
        CachedLabel* cachedLabel = mLabelCache.take(QString::fromUtf8(
            key)); // attempt to take label from cache (don't use object() because we want
                   // ownership/prevent deletion during our operations, we re-insert it afterwards)
        ...
        labelAnchor.x()+cachedLabel->offset.x()+cachedLabel->pixmap.width()/mParentPlot->bufferDevicePixelRatio()
        > viewportRect.right() || labelAnchor.x()+cachedLabel->offset.x() < viewportRect.left();
          else
            labelClippedByBorder =
        labelAnchor.y()+cachedLabel->offset.y()+cachedLabel->pixmap.height()/mParentPlot->bufferDevicePixelRatio()
        > viewportRect.bottom() || labelAnchor.y()+cachedLabel->offset.y() < viewportRect.top();
        }
        */
        if (!labelClippedByBorder)
        {
            ...
            finalSize = cachedLabel->pixmap.size()
                / mParentPlot
                      ->bufferDevicePixelRatio(); // TODO: collect this in a member rect list?
        }
    ...
    else // label caching disabled, draw text directly on surface:
    {
       ...
       > viewportRect.right() || finalPosition.x()+labelData.rotatedTotalBounds.left() <
       viewportRect.left(); else labelClippedByBorder =
       finalPosition.y()+(labelData.rotatedTotalBounds.height()+labelData.rotatedTotalBounds.top())
       > viewportRect.bottom() || finalPosition.y()+labelData.rotatedTotalBounds.top() <
       viewportRect.top();
       }
       */
        if (!labelClippedByBorder)
...
QPointF QCPLabelPainterPrivate::getAnchorPos(const QPointF& tickPos)
{
    switch (mAnchorMode)
    {
        case amRectangular:
        {
            switch (mAnchorSide)
            {
                ...
                case asBottomLeft:
                    return tickPos + QPointF(mPadding * M_SQRT1_2, -mPadding * M_SQRT1_2);
                default:
                    qDebug() << Q_FUNC_INFO << "invalid mode for anchor side: " << mAnchorSide;
                    break;
            }
            break;
        }
        ...
        default:
            qDebug() << Q_FUNC_INFO << "invalid mode for anchor mode: " << mAnchorMode;
            break;
    }
    return tickPos;
}
                                      ...
                                      const LabelData& labelData) const
{
    ...
    painter->drawLine(QLineF(0, baseline-mLetterCapHeight, labelData.totalBounds.width(),
    baseline-mLetterCapHeight)); painter->restore();
    */
...
void QCPLabelPainterPrivate::applyAnchorTransform(LabelData& labelData) const
{
    if (!qFuzzyIsNull(labelData.rotation))
        labelData.transform.rotate(
            labelData.rotation); // rotates effectively clockwise (due to flipped y axis of painter
                                 // vs widget coordinate system)
    ...
    labelData.transform.translate(
        0,
        -labelData.totalBounds.height() + mLetterDescent
    ...
    else if (labelData.side == asTop
             || labelData.side == asBottom) // anchor is centered horizontally
...
*/
/*
void QCPLabelPainterPrivate::getMaxTickLabelSize(const QFont &font, const QString &text,  QSize
...
QCPLabelPainterPrivate::createCachedLabel(const LabelData& labelData) const
{
    ...
    // offset is between label anchor and topleft of cache pixmap, so pixmap can be drawn at
    // pos+offset to make the label anchor appear at pos. We use rotatedTotalBounds.topLeft()
    // because rotatedTotalBounds is in a coordinate system where the label anchor is at (0, 0)
...
QByteArray QCPLabelPainterPrivate::cacheKey(const QString& text, const QColor& color,
...
QCPLabelPainterPrivate::skewedAnchorSide(const QPointF& tickPos, double sideExpandHorz,
...
QCPLabelPainterPrivate::rotationCorrectedSide(AnchorSide side, double rotation) const
{
    ...
    if (!qFuzzyIsNull(rotation))
    {
        if (!qFuzzyCompare(qAbs(rotation),
                           90)) // avoid graphical collision with anchor tangent (e.g. axis line)
                                // when rotating, so change anchor side appropriately:
        {
            if (side == asTop)
        ...
        else // for full rotation by +/-90 degrees, other sides are more appropriate for centering
             // on anchor:
        {
            if (side == asLeft)
...
const QFontMetrics& QCPLabelPainterPrivate::fontMetricsFor(const QFont& font) const
{
    for (int i = 0; i < 2; ++i)
    {
