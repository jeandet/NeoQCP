...
/*! \internal
  ...
  \seebaseclassmethod
*/
QString QCPAxisTickerPi::getTickLabel(double tick, const QLocale& locale, QChar formatChar,
                                      int precision)
{
    ...
    {
        // simply construct fraction from decimal like 1.234 -> 1234/1000 and then simplify
        // fraction, smaller digits are irrelevant due to mPiTickStep conditional above
    ...
    else
    {
        if (qFuzzyIsNull(tickInPis))
            ...
            return QCPAxisTicker::getTickLabel(tickInPis, locale, formatChar, precision)
                + mPiSymbol;
    }
}
