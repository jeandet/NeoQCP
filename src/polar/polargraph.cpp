...
void QCPPolarLegendItem::draw(QCPPainter* painter)
{
    ...
    QRectF textRect = painter->fontMetrics().boundingRect(0, 0, 0, iconSize.height(),
    ...
    painter->drawText(mRect.x() + iconSize.width() + mParentLegend->iconTextPadding(), mRect.y(),
                      textRect.width(), textHeight, Qt::TextDontClip, mPolarGraph->name());
    // draw icon:
    painter->save();
...
/*! \overload
  ...
  \see addData
*/
void QCPPolarGraph::setData(const QVector<double>& keys, const QVector<double>& values,
                            bool alreadySorted)
{
...
void QCPPolarGraph::addData(const QVector<double>& keys, const QVector<double>& values,
                            bool alreadySorted)
{
...
void QCPPolarGraph::rescaleValueAxis(bool onlyEnlarge, bool inKeyRange) const
{
    ...
    bool foundRange;
    QCPRange newRange
        = getValueRange(foundRange, signDomain, inKeyRange ? keyAxis->range() : QCPRange());
    if (foundRange)
    {
        if (onlyEnlarge)
...
/* inherits documentation from base class */
QCPRange QCPPolarGraph::getValueRange(bool& foundRange, QCP::SignDomain inSignDomain,
                                ...
                                const QVariant& details, bool* selectionStateChanged)
{
    ...
    if (mSelectable != QCP::stNone)
    {
        ...
        if (additive)
        {
            if (mSelectable == QCP::stWhole) // in whole selection mode, we toggle to no selection
            ...
            else // in all other selection modes we toggle selections of homogeneously
                 // selected/unselected segments
            {
                if (mSelection.contains(newSelection)) // if entire newSelection is already
                                                       // selected, toggle selection
...
*/
void QCPPolarGraph::drawScatterPlot(QCPPainter* painter, const QVector<QPointF>& scatters,
...
double QCPPolarGraph::pointDistance(const QPointF& pixelPoint,
...
void QCPPolarGraph::getDataSegments(QList<QCPDataRange>& selectedSegments,
...
void QCPPolarGraph::getVisibleDataBounds(QCPGraphDataContainer::const_iterator& begin,
...
void QCPPolarGraph::getOptimizedLineData(QVector<QCPGraphData>* lineData,
                                         ...
                                         const QCPGraphDataContainer::const_iterator& end) const
{
    ...
    const double clipMargin = range.size()
        * 0.05; // extra distance from visible circle, so optimized outside lines can cover more
                // angle before having to place a dummy point to prevent tangents
    const double upperClipValue = range.upper
        + (reversed ? 0
                    : range.size() * 0.05
                   + clipMargin); // clip slightly outside of actual range to avoid line thicknesses
                                  // to peek into visible circle
    const double lowerClipValue = range.lower
        - (reversed ? range.size() * 0.05 + clipMargin
                    : 0); // clip slightly outside of actual range to avoid line thicknesses to peek
                          // into visible circle
    const double maxKeySkip
        = qAsin(qSqrt(clipMargin * (clipMargin + 2 * range.size())) / (range.size() + clipMargin))
        / M_PI * mKeyAxis->range().size(); // the maximum angle between two points on outer circle
                                           // (r=clipValue+clipMargin) before connecting line
                                           // becomes tangent to inner circle (r=clipValue)
    double skipBegin = 0;
    ...
    while (it != end)
    {
        ...
        {
            if (aboveRange) // jumped directly from above to below visible range, draw previous
                            // point so entry angle is correct
            {
                aboveRange = false;
                if (!reversed) // TODO: with inner radius, we'll need else case here with projected
                               // border point
            ...
            if (it->key - skipBegin
                > maxKeySkip) // add dummy point if we're exceeding the maximum skippable angle (to
                              // prevent unintentional intersections with visible circle)
            {
        ...
        {
            if (belowRange) // jumped directly from below to above visible range, draw previous
                            // point so entry angle is correct (if lower means outer, so if reversed
                            // axis)
            {
                belowRange = false;
                if (reversed)
        ...
        else // value within bounds where we don't optimize away points
        {
            if (aboveRange)
            {
                ...
                if (!reversed)
                    lineData->append(
                        *(it - 1)); // just entered from above, draw previous point so entry angle
            ...
            if (belowRange)
            {
                ...
                if (reversed)
                    lineData->append(
                        *(it - 1)); // just entered from below, draw previous point so entry angle
    ...
    if (aboveRange)
    {
        ...
        if (!reversed)
            lineData->append(
                *(it - 1)); // just entered from above, draw previous point so entry angle is
                            // correct (if above means outer, so if not reversed axis)
    }
    if (belowRange)
    {
        belowRange = false;
        if (reversed)
            lineData->append(
                *(it - 1)); // just entered from below, draw previous point so entry angle is
                            // correct (if below means outer, so if reversed axis)
    }
}

void QCPPolarGraph::getOptimizedScatterData(QVector<QCPGraphData>* scatterData,
                                            ...
                                            QCPGraphDataContainer::const_iterator end) const
{
    ...
    const double upperClipValue = range.upper
        + (reversed ? 0 : clipMargin); // clip slightly outside of actual range to avoid scatter
                                       // size to peek into visible circle
    const double lowerClipValue = range.lower
        - (reversed ? clipMargin : 0); // clip slightly outside of actual range to avoid scatter
                                       // size to peek into visible circle
    QCPGraphDataContainer::const_iterator it = begin;
    while (it != end)
    {
