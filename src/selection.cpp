...
QCPDataRange QCPDataRange::bounded(const QCPDataRange& other) const
{
    ...
    if (result.isEmpty()) // no intersection, preserve respective bounding side of otherRange as
                          // both begin and end of return value
    {
...
QCPDataSelection& QCPDataSelection::operator-=(const QCPDataRange& other)
{
    ...
    while (i < mDataRanges.size())
    {
        ...
        if (thisBegin >= other.end())
            break; // since data ranges are sorted after the simplify() call, no ranges which
                   // contain other will come after this

        if (thisEnd > other.begin()) // ranges which don't fulfill this are entirely before other
                                     // and can be ignored
        {
            ...
            {
                if (thisEnd
                    <= other.end()) // only trailing segment is encompassed, trim accordingly
                {
                ...
                else // other lies inside this range, so split range
                {
                    ...
                    break; // since data ranges are sorted (and don't overlap) after simplify()
                           // call, we're done here
                }
            }
        }
        ++i;
...
void QCPDataSelection::simplify()
{
    ...
    while (i < mDataRanges.size())
    {
        if (mDataRanges.at(i - 1).end()
            >= mDataRanges.at(i).begin()) // range i overlaps/joins with i-1, so expand range i-1
...
void QCPDataSelection::enforceType(QCP::SelectionType type)
{
    ...
    switch (type)
    {
        ...
        case QCP::stWhole:
        {
            // whole selection isn't defined by data range, so don't change anything (is handled in
            // plottable methods)
            break;
        }
        ...
        {
            // this is the selection type that allows all concievable combinations of ranges, so do
            // nothing
            break;
        }
    }
...
bool QCPDataSelection::contains(const QCPDataSelection& other) const
{
    ...
    return thisIndex
        < mDataRanges.size(); // if thisIndex ran all the way to the end to find a containing range
                              // for the current otherIndex, other is not contained in this
}
...
QCPDataSelection QCPDataSelection::inverse(const QCPDataRange& outerRange) const
{
    ...
    for (int i = 1; i < mDataRanges.size(); ++i)
        result.addDataRange(QCPDataRange(mDataRanges.at(i - 1).end(), mDataRanges.at(i).begin()),
                            false);
